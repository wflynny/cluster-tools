...
class Model(object):
    def __init__(self, **kwargs):
        ...
        self.col_names = OrderedDict((('id', 'ID'),
                          ('job_owner_full', 'User'),
                          ('job_owner_group', 'Group'),
                          ('job_name', 'Job Name'),
                          ('resource_list.nodect', 'Nodes'),
                          ('queue', 'Q'),
                          ('resources_used.walltime', 'ETime'),
                          ('resource_list.walltime', 'RTime'),
                          ('job_state', 'S')))
    ...
    def format_columns(self, **kwargs):
        self.col_names.update(**kwargs)
        return self.frame.rename(columns=self.col_names)[self.col_names.values()]
...
class Console(object):
    ...
    def handle_keypress(self, kp):
        if kp in ('q', '\x03', '\x04', '\x1a'):
            ...
            m = {'k': self.scroll_up, 'j': self.scroll_down}
            m[kp]()
        elif kp in 'SRA':
            if kp == 'A':
                self.stateidx = 0
            elif kp == 'R':
                self.stateidx = int(self.stateidx == 0)
            else:
                self.stateidx = (self.stateidx + 1) % len(self.states)
        elif kp in 's':
            pass

    def scroll_up(self):
        #self.curr_idx = min(0, self.curr_idx-1)
        self.display.move_cursor(1)
    def scroll_down(self):
        #self.curr_idx = min(self.curr_idx+1, len(self.model.frame)-self.height)
        self.display.move_cursor(0)

    def run(self, refresh=0.5):
        self.model.init_frame()
        self.display = Display()

        self.height = self.display.term.height - 2
        self.curr_idx = 0

        self.event_queue = []

        self.running = True
        self.last_update = -1000#time.time()
        ...
        while self.running:
            ...
            if time.time() - self.last_update > refresh:
                ...
                #frame = self.model.frame.iloc[self.curr_idx:set.curr_idx+self.height]
                #self.display.update_frame(self.model.frame.iloc[self.curr_idx:self.curr_idx+self.height])
            frame = self.model.format_columns()
            frame = frame[frame.S.isin(self.states[self.stateidx])]
            self.display.update_frame(frame)
            self.event_queue.append(self.get_input())

            #self.display.update_frame(self.model.frame[:10])
            #self.display.display()
...
class Display(object):
    ...
    def move_cursor(self, up):
        x, y = self.cursor_pos
        new_y = y - 2*(up-1)
        if not 0 <= new_y < self.height: return
        self.cursor_pos = (x, new_y)
        print self.term.move(*self.cursor_pos)

    def display(self):
        term = self.term
        x, y = self.cursor_pos
        window = self.frame.iloc[y:y+term.height-2]
        print term.clear()
        print term.move(*self.cursor_pos), window#.id#.toclip
